...
String response, GET, Prev, S;
boolean errorRX, AllSame;
...
void setup(){
  ...
  while (!AllSame){
    for (int i = 0; i < 3; i++){
      Prev = S;
      S = Date();
      if (S == Prev) AllSame = true;
      else AllSame = false; 
    }
  }
  if (AllSame) Serial.println("There are no errors");
  else Serial.println("There are errors");
}
